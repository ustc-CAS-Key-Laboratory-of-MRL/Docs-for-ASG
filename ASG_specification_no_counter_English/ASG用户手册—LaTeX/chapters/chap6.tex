\chapter{\heiti Troubleshooting}
This chapter lists the commonly encountered failures of ASG-GT50-C and their solutions. When you encountered whose problems, please solve following the corresponding steps. If the problem persists, please contact us.
%下面列举了ASG-GT50-C 在使用过程中可能出现的故障及排查方法。当您遇到这些故障时，请按照相应的步骤进行处理，如不能处理，请及时与我司联系。

\section{\heiti The instrument cannot be recognized}
When the instrument connects with a computer that has been installed the USB driver, but it cannot be recognized. Please reconnect the USB cord. If still fail, please contact us.
%在已经安装过USB驱动程序的计算机上连接仪器，但在计算机的设备管理器中发现无法识别设备，请重新插拔USB连接线并检查USB 连接线是否有损伤。确认USB连接线完好，并多次重新插拔后仍无法识别设备，请及时与我司联系。

\section{\heiti Settings are correct but no signal is generated}
\noindent {\textcircled{\normalsize{\textbf{1}}}}. Check whether the transmission cables are connected to the output channel fastening.
%\noindent {\textcircled{\normalsize{\textbf{1}}}}. 检查SMA线缆是否与相应的通道输出端口（OUT1 - OUT8）紧固连接。

\noindent {\textcircled{\normalsize{\textbf{2}}}}. Check the transmission cables for internal injuries.
%\noindent {\textcircled{\normalsize{\textbf{2}}}}. 检查SMA线缆是否有内部损伤。

\noindent {\textcircled{\normalsize{\textbf{3}}}}. Check whether the transmission cables are connected to the test instrument fastening.
%\noindent {\textcircled{\normalsize{\textbf{3}}}}. 检查SMA线缆是否与测试仪器紧固连接。

\noindent {\textcircled{\normalsize{\textbf{4}}}}. If all three points above have been checked, but the instrument still cannot output signal, please contact us.  
%\noindent {\textcircled{\normalsize{\textbf{4}}}}. 以上三条均检查无误后，若仍无方波信号输出，请及时与我司联系。

%\begin{enumerate}
%\item 检查定义的Counter序列是否满足要求。
%\item 检查外部输入信号是否正常，并且输入信号频率在最大计数率（50 MHz）内。
%\item 如果定义的Counter序列满足需求，且外部输入信号正常，却仍无计数结果读\\
%\end{enumerate}
